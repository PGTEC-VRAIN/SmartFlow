...
router = APIRouter(
    prefix="/IFS_ECMWF",  # Todas las rutas aquí comenzarán con /IFS_ECMWF
    tags=["IFS_ECMWF"] # Agrupa las rutas en Swagger UI
)

# Define la ruta base donde Airflow guarda los archivos
# (Asegúrate de que esta ruta esté montada en Docker)
DATA_DIR = "/app/data_files/IFS" 
MODELO = "ecmwf_ifs"
...
def find_latest_json_path(directory_path: str, filename_pattern: str) -> str:
    ...
    if not list_of_files:
        ...
        raise HTTPException(status_code=503, detail=f"Datos del modelo IFS_ECMWF no disponibles. No se encontraron archivos.")

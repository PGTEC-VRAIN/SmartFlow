"""
A REVISAR ESTE SCRIPT. NO ESTÁ INCLUIDO EN EL MAIN.PY

DE MOMENTO GUARDO LOS DATOS DE INCENDIOS EN CSV Y ESTE SCRIPT SOLO LEE JSON.

TAMPOCO CREO QUE SEA NECESARIO DEVOLVER ESTOS DATOS COMO API PARA EL TETIS

LO MEJOR SERIA GUARDAR LOS CSV EN UN BUCKET S3
"""
...
router = APIRouter(
    prefix="/EFFIS",  # Todas las rutas aquí comenzarán con /AIFS_ECMWF
    tags=["EFFIS"] # Agrupa las rutas en Swagger UI
)

# Define la ruta base donde Airflow guarda los archivos
# (Asegúrate de que esta ruta esté montada en Docker)
DATA_DIR = "/app/data_files/EFFIS" 
MODELO = "EFFIS"
...
def find_latest_json_path(directory_path: str, filename_pattern: str) -> str:
    ...
    if not list_of_files:
        ...
        raise HTTPException(status_code=503, detail=f"Datos del modelo EFFIS no disponibles. No se encontraron archivos.")

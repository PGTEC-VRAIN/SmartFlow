...
def ejecutar_descarga_aifs():
    ...
    print("FIN DEL PROCESO DE DESCARGA DE PREDICCIONES AIFS de ECMWF.")
    print("\n FIN")

with DAG(
    dag_id='AIFS_0.25º_ECMWF_CV',
    default_args=default_args,
    description='Descarga y procesamiento de predicciones AIFS de ECMWF para la Comunidad Valenciana',
    schedule='0 */1 * * *',  # Cada 1 hora
    start_date=datetime(2025, 10, 1),
    catchup=False,
    tags=['AIFS', 'ECMWF', 'meteorología', '0.25º'],
) as dag:
    
    # 1. Tarea para ejecutar el procesamiento y descarga
    procesar_ecmwf_aifs025 = PythonOperator(
        task_id='procesar_ecmwf_aifs025',
        python_callable=ejecutar_descarga_aifs, # Llama a la función Python
    )
    procesar_ecmwf_aifs025

...
def ejecutar_descarga_arpege():
    ...
    variables_a_limpiar = [
    var for var in VARIABLES if var in columnas_existentes
]
    df = df.dropna(subset=variables_a_limpiar) # Borra las filas con datos de las variables faltantes que corresponden a predicciones no disponibles.
    ...
    relativeHumidity = df["relative_humidity_2m"].to_list(),
    solarRadiation = df["shortwave_radiation"].to_list(),
)
    datos_dict = datos._to_dict()
    #df.to_csv("./datos_ARPEGE.csv")
...
with DAG(
    dag_id='ARPEGE_METEOFRANCE_CV',
    default_args=default_args,
    description='Descarga y procesamiento de predicciones del modelo ARPEGE de MeteoFrance para la Comunidad Valenciana',
    schedule='0 */1 * * *',  # Cada 1 hora
    start_date=datetime(2025, 10, 1),
    catchup=False,
    tags=['meteofrance', 'meteorología', 'arpege'],
) as dag:
    
    # 1. Tarea para ejecutar el procesamiento y descarga
    procesar_arpege_meteofrance = PythonOperator(
        task_id='procesar_arpege_meteofrance',
        python_callable=ejecutar_descarga_arpege, # Llama a la función Python
    )
    procesar_arpege_meteofrance
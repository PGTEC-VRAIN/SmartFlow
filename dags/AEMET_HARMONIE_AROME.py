...
def download_harmonie_pb(output_dir):
    """
    Descarga el archivo ZIP del modelo HARMONIE desde AEMET,
...
def tif_to_dataframe_realvalues(filepath, variable):
    ...
    timestamp = datetime.strptime(match.group(1), "%Y-%m-%dT%H") if match else None
...
with DAG(
    dag_id='AEMET_HARMONIE_AROME_CV',
    default_args=default_args,
    description='Descarga y procesamiento de predicciones HARMONIE-AROME (AEMET) para la Comunidad Valenciana',
    ...
    procesar_aemet_harmonie = PythonOperator(
        ...
        python_callable=ejecutar_descarga_aemet, # Llama a la función Python
    )
    procesar_aemet_harmonie
...
DATA_DIR = Path("/opt/airflow/data/GFS/")
...
with DAG(
    ...
    description='Descarga y procesamiento de predicciones GFS de NOAA para la Comunidad Valenciana',
    schedule='0 */1 * * *',  # Cada 1 hora
    start_date=datetime(2025, 10, 1),
    catchup=False,
    tags=['GFS', 'NOAA', 'meteorología', 'global'],
) as dag:
    
    # 1. Tarea para ejecutar el procesamiento y descarga
    procesar_gfs_noaa = PythonOperator(
        task_id='procesar_gfs_noaa',
        python_callable=ejecutar_descarga_gfs, # Llama a la función Python
    )
    procesar_gfs_noaa
